...
const signup = async (req,res) =>{
  ...
  try {
   ...
   // Vulnerable version: password stored in plaintext for security demonstration
    await User.create({
      name,
      email,
      password,
...
//}
    ...
    try{
      //vulnerable version: directly using email and password for finding user to create a security vulnerability for demonstration purposes. 
...
const isMatch = await password === user.password; //vulnerable version: directly comparing plaintext passwords for demonstration purposes, which is not secure and should never be used in production applications. Always use a secure hashing algorithm like bcrypt to hash passwords and compare hashes instead of plaintext values.
...
if (!isMatch) {
  ...
  return res.status(401).json({
    message: "Invalid credentials"
      ...
      //vulnerable version: without expiration time for demonstartion purposes
        const accesstoken = jwt.sign({userId : user._id,  role: user.role},process.env.secret_key);  //jwt.sign(payload, secret, options)
